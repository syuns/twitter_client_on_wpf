...
namespace Client.Library.OAuth {
	public class oAuthTwitter : OAuthBase {
		...
		public bool Authorized() {
			return !(string.IsNullOrEmpty(this.ConsumerKey) || string.IsNullOrEmpty(this.ConsumerSecret) ||
				string.IsNullOrEmpty(this.Token) || string.IsNullOrEmpty(TokenSecret));
		}

		public bool ConvertToOAuthUrl(Method method, ref string url, ref string postData) {
			if (!Authorized()) {
				return false;
			}
		...
		public void Reset() {
			ConsumerKey = ConsumerSecret = OAuthToken = Token = TokenSecret = PIN = ScreenName = String.Empty;
		}
	}
}
